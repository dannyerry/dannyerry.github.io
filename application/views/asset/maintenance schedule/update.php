...
<div class="col-sm-10 col-sm-offset-2">
    ...
    <form method="post" id="myForm" action="<?= base_url('asset/MaintenanceVendor/do_update')?>">
		<section>		
			...
			<table class="master-table" id="myTable">
				<tr>
					...
					<td><?= $maintenance_vendor->contract_number; ?></td>
				</tr>
				<tr>
						...
						<select class="form-control" name="contract_category" id="contract_category" class="js-example-basic-single">
							...
							<?php
								foreach($contract_cat as $c){
									if($maintenance_vendor->contract_category_id == $c->contract_id){
							?>
										<option value="<?= $c->contract_id ?>" selected="selected"><?= $c->contract_category_name ?></option>
							<?php
									}else{
							?>
						...
						<select class="form-control" name="vendor_name" id="vendor_name" class="js-example-basic-single">
							...
							<?php
								foreach($vendor_name as $vn){
									if($maintenance_vendor->vendor_id == $vn->vendor_id){
							?>			
										<option value="<?= $vn->vendor_id ?>" selected="selected"><?= $vn->vendor_name ?></option>	
							<?php
									}else{
				...
				<tr>
					...
					<td><textarea class="form-control" name="contratc_desc" placeholder="Contract Description .. " required><?= $maintenance_vendor->contract_desc; ?></textarea></td>
				</tr>
				<tr>
					<td>Contract Signed</td>
					<td><input type="text" name="contract_signed" class="form-control" placeholder="Contract Signed" value="<?= $maintenance_vendor->contract_signed; ?>" required></td>
				</tr>
				<tr>
					<td>Contract Expired</td>
					<td><input type="text" name="contract_expired" class="form-control" placeholder="Contract Expired" value="<?= $maintenance_vendor->contract_expired; ?>" required></td>
				</tr>
				<tr>
					<td>Contract Value</td>
					<td><input type="text" name="contract_value" class="form-control" placeholder="Contract Value" value="<?= $maintenance_vendor->contract_value; ?>" required></td>
				</tr>
			</table>
		...
		<section>
			<div class="section-content"><br />
				<input type="hidden" name="id" value="<?= $maintenance_vendor->maintenance_vendor_id;?>">
