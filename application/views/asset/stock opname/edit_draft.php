...
<div class="col-sm-10 col-sm-offset-2">
    ...
    <form method="post" >
		<input type="text" name="id_stock_opname" id="id_stock_opname" value="<?=$stock_opname->stock_opname_id?>">
		<section >		
			...
			<table class="master-table" id="myTable">
				<tr>
					<td>Waarehouse</td>
					<td>
						<select class="form-control warehouseplace" ng-model="warehouseplace" name="warehouseplace" onchange="excecuteSearch()">
							<option value="">Choose Warehouse Location</option>
							<?php
								foreach($warehouse as $w){ 
									if($stock_opname->warehouse_id==$w->warehouse_id){
							?>
										<option value="<?=$w->warehouse_name?>~<?=$w->warehouse_id?>" selected><?=$w->warehouse_location?></option>
							<?php
									}else{
							?>			
										<option value="<?=$w->warehouse_name?>~<?=$w->warehouse_id?>"><?=$w->warehouse_location?></option>
							<?php
									}	
								}
							?>
...
$(document).ready(function(){
	...
	if(ware != ''){
		...
		ware=warehousename[0];
		$("#submittername").text(ware);
	}
});
...
<script>
	function do_insert(xy){
		var ware= $('.warehouseplace').val();
		var id= $('#id_stock_opname').val();
		var warehousename=ware.split('~');
		ware=warehousename[1];
		var dataIn = arrayDataInsertInventory;
		var dataAs = arrayDataInsertAsset;
		var dataString = 'gudang='+ware+'&dataArrayAsset='+dataAs+'&dataArrayInventory='+dataIn+'&jenis='+xy+'&id='+id;	
		$.ajax({
			type:'POST',
			data:dataString,
			url:'<?= base_url("asset/stockopname/do_edit_draft") ?>',
